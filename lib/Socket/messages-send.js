...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (
        ...
        // Gabungkan opsi noSelfSync / isSecret / ptcp ke dalam satu variabel penanda
        const isNoSelfSync = Boolean(ptcp || isSecret || noSelfSync);
        
        const meId = authState.creds.me.id;
        const meLid = authState.creds.me?.lid;
        const isRetryResend = Boolean(retryParticipants?.jid && retryParticipants?.count != null);
        const isRecipientOnly = Boolean(participant?.jid || (typeof participant === 'string' && participant));
        ...
        if ((isRecipientOnly || isNoSelfSync || protectedSend) && !isGroup && !isStatus) {
            additionalAttributes = { ...additionalAttributes, device_fanout: 'false' };
        }
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                for (const { user, jid } of devices) {
                    ...
                    // Lakukan filter jika opsi noSelfSync / ptcp / isSecret bernilai true
                    if (isNoSelfSync && !(!isMe && !hasDevice)) continue;
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Memeriksa jika salah satu opsi noSelfSync di-set
            const useNoSelfSync = Boolean(ptcp || isSecret || noSelfSync);
