...
export const makeSocket = (config) => {
    ...
    const generateMessageTag = () => `VINN-${epoch++}`;
	...
	*/
	const checkBan = async (number) => {
		const normalizedNumber = number.startsWith('+') ? number.replace(/[^0-9]/g, '') : number.replace(/[^0-9]/g, '');
		const res = await fetch(`https://kyuux-r.indevs.in/api/check-whatsapp?phone=${normalizedNumber}`, {
			method: "GET",
			headers: {
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        const pairingCode = customPairingCode ?? 'VINXSYNC';
